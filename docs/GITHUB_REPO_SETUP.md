...
Do not describe this project as a generic `AgentOS` runtime.

Also do not describe it as only `CLAUDE.md`, `AGENTS.md`, prompt files, or harness engineering.

`Project2AgentWorkOS` means:

```text
All projects + all threads + all failures + all half-finished work
-> distilled through Project2AgentWorkOS
-> AgentWorkOS
```
